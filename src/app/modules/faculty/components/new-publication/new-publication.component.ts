...
export class NewPublicationComponent extends BaseComponent implements OnInit {
    ...
    try {
      ...
      if (this.dialogContext.data.edit) {
        publication = await lastValueFrom(
          this.facultyService.updatePublication(this.dialogContext.data.id, data)
        );
      }
  ...
  private async getPublicationTypes() {
    try {
      this.publicationTypes = await lastValueFrom(this.facultyService.getPublicationTypes());
  ...
  private async getPublicationFromScopus(query: any) {
    try {
      const publication = await lastValueFrom(this.facultyService.getPublicationFromScopus(query));
