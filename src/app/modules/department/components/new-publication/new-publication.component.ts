...
import { PUBLICATION_TYPES } from "src/app/modules/teacher/models/constants";
import { Publication } from "src/app/modules/teacher/models/publication";
import { NewPublication } from "src/app/modules/teacher/models/publication-new";
...
export class NewPublicationComponent extends BaseComponent implements OnInit {
  ...
  constructor(
    fb: FormBuilder,
    private teacherService: DepartmentService,
    ...
    this.form = fb.group({
      ...
      publicationYear: [this.currentYear, [Validators.required, Validators.max(this.currentYear)]],
      startPage: [null, Validators.required],
      endPage: [null, Validators.required],
  ...
  public async save() {
    ...
    if (this.isScopus) {
      if (!user.ieeeXploreAuthorName) {
        this.commonDialogService.openWarningDialog("Потрібно додати ім'я автора в IEEE Xplore");
        return;
      }
    ...
    try {
      ...
      if (this.dialogContext.data.edit) {
        publication = await lastValueFrom(
          this.teacherService.updatePublication(this.dialogContext.data.id, data)
        );
      }
  ...
  private async getPublicationTypes() {
    try {
      this.publicationTypes = await lastValueFrom(this.teacherService.getPublicationTypes());
  ...
  private async getPublicationFromScopus(query: any) {
    try {
      const publication = await lastValueFrom(this.teacherService.getPublicationFromScopus(query));
  ...
  private setData() {
    ...
    this.isScopus = this.dialogContext.data.typeValue === PUBLICATION_TYPES.scopus;
  }
  ...
  private subscribeToChanges() {
    ...
    this.form.controls.articleNumber.valueChanges
      ...
      .subscribe(async (articleNumber) => this.onArticleNumberChange(articleNumber));
