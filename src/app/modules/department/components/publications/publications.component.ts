...
export class PublicationsComponent extends BaseComponent implements OnInit {
  ...
  public async deletePublication() {
    ...
    if (result) {
      try {
        await lastValueFrom(this.teacherService.deletePublication(this.selectedPublication.id));
  ...
  private async getPublications() {
    try {
      this.publications = await lastValueFrom(this.teacherService.getDepartmentPublications());
  ...
  private async loadScientificJournalsCategoryB() {
    try {
      await lastValueFrom(this.teacherService.loadScientificJournalsCategoryB());
    } catch (err: any) {
      this.teacherService.showRequestError(err);
    }
  }
